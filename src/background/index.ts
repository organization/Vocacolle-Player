...
const extractType = (url: string): [number, string] | null => {
  const [, year, season] = url.match(/vocaloid-collection\.jp\/([0-9]{4})-([^/]+)/) ?? [];
...
const onUpdate = async (url: string) => {
  ...
  const iconURL = type ? `https://vocaloid-collection.jp/${type[0]}-${type[1]}/favicon.ico` : 'https://vocaloid-collection.jp/favicon.ico';
